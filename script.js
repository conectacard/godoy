const CONFIG = {
    youtubeUrl: "https://www.youtube.com/watch?v=t33y1RHjsZQ",
    ...
    get textos() {
        ...
        return {
            cat2: { 
                ...
                c: tObj.cat2_c || "Trabaja cada día en su preparación actoral y mental, aplicando una mentalidad de cero límites y perfeccionando cada interpretación para llevar el nombre de México a lo más alto." 
            },
            cat3: { 
                t: tObj.cat3_t || "VIDEOS DE MI TRAYECTORIA", 
                c: tObj.cat3_c || "Su mayor fortaleza es la resiliencia mental combinada con la disciplina absoluta en cada interpretación." 
            }
        };
    }
};
    ...
    if (cat === 'cat3' || cat === 'cat2') {
        ...
        if (cat === 'cat3') {
            ...
            videoContainer.innerHTML = `
                ...
                <a href="https://www.youtube.com/shorts/PRYkPH2fAoc" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_tributo_tri || 'Asi llegué a ONE PIECE'}</a>
                <a href="https://www.youtube.com/shorts/K3h6EDJD2R0" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_junto_mar || 'Me enteré de que sería Luffy'}</a>
                <a href="https://www.youtube.com/shorts/uAGMTsfZvjo" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_con_gente || 'En ONE PIECE'}</a>
                <a href="https://www.youtube.com/shorts/unhhxhYenkM" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #000; color: #fff; padding: 12px 20px; border-radius: 8px; text-decoration: none; font-weight: 800; font-size: 0.85rem; text-align: center; border: 1px solid var(--brand-accent);">${tObj.btn_cantando_beatles || 'Iñaki de niño'}</a>
            `;
        } else if (cat === 'cat2') {
            const lang = localStorage.getItem('selectedLanguage') || 'es';
            const tObj = (typeof translations !== 'undefined' && translations[lang]) ? translations[lang] : {};
            ...
            videoContainer.innerHTML = `
                <a href="https://www.youtube.com/shorts/L9TlWB01rMY" target="_blank" style="display: block; width: 100%; box-sizing: border-box; background: #b00; color: #fff; padding: 14px 25px; border-radius: 12px; text-decoration: none; font-weight: 900; font-size: 0.9rem; text-align: center;">
                    ${tObj.btn_video_historia || 'MI HISTORIA'}
                </a>
            `;
        }
